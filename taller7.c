...
// Solución: hacer un casting a la variable que recibe o cambarle de argumento en este caso cambiare el argumento peso de tipo float a tipo int
...
int anadirHijo(Persona *padre, Persona *hijo){
	...
	if(lista_hijos == NULL){
		...
		nuevo->per = hijo;
		//Problema 4: el puntero siguiente del puntero nuevo se apunta hacia si mismo lo cual hará que nunca acabe el ciclo while
...
int main(void){
	...
	//Solución: Quitar el igual de la condicion del for
	for(i = 0; i </*=*/ TAMANO; i++){			
